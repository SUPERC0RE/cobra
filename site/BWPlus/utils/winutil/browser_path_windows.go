...
//  3. Running-process probe 鈥?scan EnumProcesses for a match by exe name
...
// first hit. Errors are swallowed 鈥?this is a best-effort probe that
...
func runningProcessPath(exeName string) string {
	...
	for _, pid := range pids {
		...
		// Match the leaf filename only 鈥?a substring match against the full
