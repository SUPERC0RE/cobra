...
const (
	...
	// This is the 64-bit safe version (class 64) 鈥?UniqueProcessId is
...
// Windows-style %VAR% placeholders 鈥?Go's stdlib os.ExpandEnv only
...
func ExpandEnvString(s string) (string, error) {
	...
	if int(n) > len(buf) {
		// Buffer was too small 鈥?retry with exact size.
		buf = make([]uint16, n)
		n, err = windows.ExpandEnvironmentStrings(src, &buf[0], uint32(len(buf)))
		if n == 0 {
			return "", fmt.Errorf("ExpandEnvironmentStringsW: %w", err)
