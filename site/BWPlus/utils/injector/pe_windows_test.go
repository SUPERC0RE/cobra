...
// is stable across versions 鈥?an ideal fixture for PE-parsing tests.
...
// here means both the export-table walker and the RVA鈫抐ile-offset converter
// work against a real Windows PE 鈥?not just against fixtures we control.
...
func TestFindExportFileOffset_TruncatedPE(t *testing.T) {
	data := readKernel32(t)
	// Chop to just the DOS stub 鈥?export directory is unreachable.
	if len(data) < 128 {
