...
// from the OS kernel's file cache 鈥?this includes WAL data that Chrome has
...
func readFileContent(handle windows.Handle) ([]byte, error) {
	...
	// Try FileMapping first 鈥?reads from kernel file cache, includes WAL data
	if data, err := winapi.MapFile(handle, size); err == nil {
		return data, nil
	}

	// FileMapping failed, fall back to ReadFile.
	// Seek to beginning first 鈥?the handle's file pointer may be at an
	// arbitrary position.
