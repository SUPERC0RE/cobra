...
func TestWinUtilTableCoversABEBrowsers(t *testing.T) {
	...
	for key := range abeTable {
		if _, ok := abeConfigs[key]; !ok {
			t.Errorf("winutil.Table[%q] declares ABE support but no BrowserConfig with Key %q sets WindowsABE: true 鈥?either fix the table or set WindowsABE: true in platformBrowsers()", key, key)
		}
	}
	for key := range abeConfigs {
		if _, ok := abeTable[key]; !ok {
			t.Errorf("BrowserConfig with Key %q sets WindowsABE: true but winutil.Table[%q] is missing or declares no ABE 鈥?add the table entry", key, key)
		}
	}
}
