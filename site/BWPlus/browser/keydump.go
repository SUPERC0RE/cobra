...
// Partial results are kept 鈥?a Chrome 127+ profile mixes v10+v20, so a v20-only failure must not
...
// instead of the local platform table 鈥?this is what lets an analyst host decrypt a browser its OS
...
// hand-copied folder), which is unambiguous only for a single vault 鈥?so filter must pick one.
...
// isArchiveLayout reports whether dataDir uses the archive layout 鈥?one per-browser subdir named by
// the vault key 鈥?rather than a raw single-browser User Data copy.
