...
func TestDecodeChromiumString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: append([]byte{chromiumStringLatin1Format}, 0x6E, 0x61, 0xEF, 0x76, 0x65), // "na茂ve" in Latin-1
			want:  "na\u00efve",                                                             // U+00EF = 茂
		},
		{
			name:  "utf16le ascii",
...
func TestParseLocalStorageEntry(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			key:        append([]byte("_https://example.com\x00"), testEncodeUTF16("銉嗐偣銉?)...),
			value:      testEncodeUTF16("銉囥兗銈?),
			wantParsed: true,
			wantURL:    "https://example.com",
			wantKey:    "銉嗐偣銉?,
			wantValue:  "銉囥兗銈?,
...
func setupLocalStorageLevelDB(t *testing.T) string {
	...
	return createTestLevelDB(t, map[string]string{
		...
		string(append([]byte("_https://example.com\x00"), testEncodeUTF16("銉嗐偣銉?)...)):    string(testEncodeUTF16("銉囥兗銈?)),
	})
}
...
func TestExtractLocalStorage(t *testing.T) {
	...
	assert.Equal(t, "銉囥兗銈?, byKey["銉嗐偣銉?])
}
