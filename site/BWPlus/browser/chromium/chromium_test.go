...
func TestNewBrowsers(t *testing.T) {
	tests := []struct {
		...
		wantCats     map[string][]string // profile 鈫?expected category base names (spot check)
...
func TestGetMasterKeys(t *testing.T) {
	// getMasterKeys routes through masterkey.NewMasterKeys on every platform 鈥?the V10 mock
	...
	tests := []struct {
		...
		retriever         masterkey.Retriever // nil 鈫?don't call SetRetrievers
		wantV10           []byte
...
// analogous Linux v10/v11 case 鈥?no platform silently drops a tier any more.
func TestGetMasterKeys_AllTiersInvoked(t *testing.T) {
	...
	assert.True(t, v10mock.called, "V10 retriever must be called 鈥?no silent bypass")
	assert.True(t, v11mock.called, "V11 retriever must be called 鈥?no silent bypass")
	assert.True(t, v20mock.called, "V20 retriever must be called 鈥?no silent bypass")
...
// TestGetMasterKeys_WindowsABEThreading pins cfg.WindowsABE 鈫?hints.WindowsABEKey threading. A
// regression here silently disables Windows ABE decryption with no dev-box-test signal 鈥?only the
// windows-tunnel sandbox 574-cookie regression would catch it 鈥?so it must be pinned at unit level.
...
func TestExtract(t *testing.T) {
	...
	tests := []struct {
		name          string
		retriever     masterkey.Retriever // nil 鈫?don't call SetRetriever
...
func TestCountEntries(t *testing.T) {
	...
	// No retriever set 鈥?CountEntries should still work (no decryption needed).
...
func TestCountEntries_NoRetrieverNeeded(t *testing.T) {
	...
	// No retriever set 鈥?CountEntries succeeds without master key.
