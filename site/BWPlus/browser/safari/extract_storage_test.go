...
func TestReadOriginBlock_FirstParty(t *testing.T) {
	...
	assert.Equal(t, uint16(0), top.port, "port 0 鈬?scheme default")
...
func TestReadOriginBlock_Latin1HighByte(t *testing.T) {
	...
	// bytes decode as Latin-1 (茅 = 0xE9) rather than being passed through as invalid UTF-8.
	data := []byte{
		0x04, 0x00, 0x00, 0x00, 0x01, 'h', 't', 't', 'p', // scheme "http"
		0x04, 0x00, 0x00, 0x00, 0x01, 'c', 'a', 'f', 0xe9, // host "caf茅" (Latin-1)
		0x00, // port default
	}
	ep, _, err := readOriginBlock(data, 0)
	require.NoError(t, err)
	assert.Equal(t, "http", ep.scheme)
	assert.Equal(t, "caf茅", ep.host)
}

func TestDecodeLatin1(t *testing.T) {
	assert.Equal(t, "caf茅", decodeLatin1([]byte{'c', 'a', 'f', 0xe9}))
...
func TestReadOriginFile_FallbackToTop(t *testing.T) {
	// Write only the top-frame block 鈥?no frame follows. Extractor should still succeed by
...
func TestDecodeUTF16LE(t *testing.T) {
	...
	t.Run("cjk", func(t *testing.T) {
		assert.Equal(t, "浣犲ソ涓栫晫", decodeUTF16LE(encodeUTF16LE("浣犲ソ涓栫晫")))
	})
	t.Run("mixed", func(t *testing.T) {
		assert.Equal(t, "hello 涓栫晫 馃實", decodeUTF16LE(encodeUTF16LE("hello 涓栫晫 馃實")))
	})
...
func TestDecodeLocalStorageValue_Truncates(t *testing.T) {
	// 1100 chars 脳 2 bytes = 2200 bytes, over the 2048 cap.
...
// ---------------------------------------------------------------------------
// extractLocalStorage 鈥?end-to-end over real nested layout fixtures
// ---------------------------------------------------------------------------
...
func TestExtractLocalStorage_CJKAndEmoji(t *testing.T) {
	...
	assert.Equal(t, "寮犱笁", values["鍚嶅瓧"])
	assert.Equal(t, "hello 涓栫晫 馃實", values["status"])
}
...
func TestExtractLocalStorage_SkipsFrameDirsWithoutDB(t *testing.T) {
	// Partition dirs that only have "origin" but no LocalStorage/ subdir must not error out 鈥?	// real Safari has plenty of these (cookies-only partitions).
	root := filepath.Join(t.TempDir(), "Origins")
...
// ---------------------------------------------------------------------------
// NULL-key handling 鈥?readLocalStorageFile / countLocalStorageFile both skip NULL keys,
