...
func TestDiscoverSafariProfiles_OrphanUUIDWithoutDBEntry(t *testing.T) {
	...
	// the DB 鈥?the orphan stays hidden because production filters profiles
...
func TestDiscoverSafariProfiles_EmptyDBIsAuthoritative(t *testing.T) {
	...
	// ReadDir fallback 鈥?the DB is the authoritative source of truth.
...
func TestDiscoverSafariProfiles_MissingDBFallsBackToReadDir(t *testing.T) {
	// SafariTabs.db absent 鈫?enumerate Safari/Profiles/ and synthesize names.
...
func TestDiscoverSafariProfiles_UUIDCaseNormalisation(t *testing.T) {
	...
	// WebKit/WebsiteDataStore uses lowercase 鈥?we must carry both.
...
func TestDiscoverSafariProfiles_EmptyProfileDirectoryFiltersOutInNewBrowsers(t *testing.T) {
	...
	got := discoverSafariProfiles(legacyHome)
	require.Len(t, got, 2) // discovery includes it 鈥?	paths := resolveSourcePaths(buildSources(got[1]))
	assert.Empty(t, paths) // 鈥?but no supported data resolves for it.
}
...
func TestResolveProfileName(t *testing.T) {
	tests := []struct {
		...
		{"涓枃", "AAAAAAAA-0000-0000-0000-000000000000", "涓枃"},
	}
...
func TestBuildSources_NamedProfile(t *testing.T) {
	...
	// Download points at the shared plist 鈥?filtering by DownloadEntryProfileUUIDStringKey
