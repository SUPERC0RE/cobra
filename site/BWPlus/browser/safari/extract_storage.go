...
//
//	<root>/<top-frame-hash>/<frame-hash>/origin                         鈥?binary; encodes top+frame origins
//	<root>/<top-frame-hash>/<frame-hash>/LocalStorage/localstorage.sqlite3
//
// top-hash == frame-hash 鈬?first-party; they differ for third-party partitioned storage.
...
// 0x00  鈫?port is the scheme default (stored as 0).
// 0x01  鈫?next two bytes are a uint16_le port.
const (
...
// parsing origin files or decoding values 鈥?CountEntries callers only need the total, not the
...
func countLocalStorageFile(path string) (int, error) {
	// mode=ro (no immutable) so SQLite replays the copied -wal sidecar 鈥?this surfaces entries
...
func readOriginFile(path string) (string, error) {
	...
	if ferr != nil {
		// Partitioned info unavailable 鈥?attribute to the top-frame origin.
		frame = top
	}
...
// 1:1 to Unicode code points U+0000鈥揢+00FF. Mirrors the helper in chromium/extract_storage.go.
...
func readLocalStorageFile(path string) ([]localStorageItem, error) {
	// mode=ro (no immutable) 鈥?see countLocalStorageFile for the WAL-replay rationale; the same
