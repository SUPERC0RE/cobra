...
func TestExtractExtensions_AppAndWebMerged(t *testing.T) {
	...
	// Results are sorted by key, so 1Password (com.1鈥? comes before iina (com.c鈥?.
	...
	// AppExtensions omit the Enabled field 鈥?defaults to true (present == enabled).
...
func TestExtractExtensions_BundleIDFallbackOnUnexpectedKey(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, got, 1)
	// Regex miss 鈫?fall back to the full trimmed key.
