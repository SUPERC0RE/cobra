...
// (WebKit/WebsiteDataStore/<uuid>) UUID forms a named profile needs. Both empty 鈬?default profile.
...
// Canonical 8-4-4-4-12 hex UUID 鈥?format check only, no semantic parse.
...
// try the ReadDir fallback; (slice, nil) 鈥?possibly empty 鈥?is authoritative.
...
// disambiguateNames appends "-2", "-3", 鈥?to duplicate names, in place.
