...
func TestExtractPasswords_DecryptFailureKeepsEntry(t *testing.T) {
	// Invalid base64 鈥?decryptPBE fails, but entry is still kept with empty user/pwd
	json := `{
		"logins": [
			{
	...
	assert.Empty(t, got[0].Username) // decrypt failed 鈫?empty
	assert.Empty(t, got[0].Password) // decrypt failed 鈫?empty
}
