...
// TestNewBrowsers_ProfilesIni verifies that profiles declared in profiles.ini
// are discovered even when stored at custom absolute paths (IsRelative=0),
// which the plain subdirectory scan can never see.
func TestNewBrowsers_ProfilesIni(t *testing.T) {
	writeIni := func(path, profiles string) {
		ini := "[General]\nStartWithLastProfile=1\n\n" + profiles
		require.NoError(t, os.WriteFile(path, []byte(ini), 0o644))
	}
	discover := func(userDataDir string) []string {
		b, err := NewBrowser(types.BrowserConfig{Name: "Firefox", Kind: types.Firefox, UserDataDir: userDataDir})
		require.NoError(t, err)
		if b == nil {
			return nil
		}
		var names []string
		for _, p := range b.profiles {
			names = append(names, filepath.Base(p.profileDir))
		}
		return names
	}

	t.Run("custom absolute profile, ini next to profile dirs", func(t *testing.T) {
		root := t.TempDir()
		mkFile(root, "abc123.default-release", "places.sqlite")
		custom := filepath.Join(root, "custom-profile")
		mkFile(custom, "logins.json")
		writeIni(filepath.Join(root, "profiles.ini"),
			"[Profile0]\nName=default-release\nIsRelative=1\nPath=abc123.default-release\n\n"+
				"[Profile1]\nName=custom\nIsRelative=0\nPath="+custom+"\n")

		assert.ElementsMatch(t, []string{"abc123.default-release", "custom-profile"}, discover(root))
	})

	t.Run("ini in parent dir (macOS layout)", func(t *testing.T) {
		root := t.TempDir()
		profDir := filepath.Join(root, "Profiles")
		mkFile(profDir, "m1n2o3.default-release", "cookies.sqlite")
		custom := filepath.Join(root, "custom-profile")
		mkFile(custom, "places.sqlite")
		writeIni(filepath.Join(root, "profiles.ini"),
			"[Profile0]\nName=default-release\nIsRelative=1\nPath=Profiles/m1n2o3.default-release\n\n"+
				"[Profile1]\nName=custom\nIsRelative=0\nPath="+custom+"\n")

		assert.ElementsMatch(t, []string{"m1n2o3.default-release", "custom-profile"}, discover(profDir))
	})
}
...
func TestResolveSourcePaths_Partial(t *testing.T) {
	...
	// Only places.sqlite exists 鈫?History, Download, Bookmark resolved
