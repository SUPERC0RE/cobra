...
func TestNewMasterKeys_Matrix(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "tier returns (nil, nil) 鈥?not applicable, silent",
			v10:     &recordingRetriever{key: k10},
			v20:     &recordingRetriever{}, // ABERetriever on non-ABE fork
			wantV10: k10,
		},
		{
			name: "all tiers (nil, nil) 鈥?no keys, no errors",
			v10:  &recordingRetriever{},
			v11:  &recordingRetriever{},
			v20:  &recordingRetriever{},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Every configured retriever must be called exactly once 鈥?this is the property
...
func TestNewMasterKeys_AllNilRetrievers(t *testing.T) {
	// All slots nil 鈥?macOS/Linux with no retriever wiring, or Windows with neither tier set up.
...
func TestNewMasterKeys_PartialNil(t *testing.T) {
	// Only V10 wired 鈥?typical macOS shape. V11/V20 left nil.
...
func TestNewMasterKeys_ErrorWrapping(t *testing.T) {
	// errors.Is should traverse errors.Join to find the original error 鈥?useful for callers
