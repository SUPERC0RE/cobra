...
func TestDefaultRetrievers_Linux(t *testing.T) {
	...
	// V10 slot: peanuts-derived kV10Key 鈥?PosixRetriever.
	assert.IsType(t, &PosixRetriever{}, r.V10, "V10 slot should hold PosixRetriever (peanuts kV10Key)")

	// V11 slot: D-Bus keyring kV11Key 鈥?DBusRetriever.
	...
	// fail in test env, which is fine 鈥?we only want to confirm the wiring, not real keys).
