...
// RetrieveKey returns (nil, nil) on failure so ChainRetriever falls through silently 鈥?the common
...
func (r *SecurityCmdRetriever) retrieveKeyOnce(storage string) ([]byte, error) {
	// Silent-run guard: only query the keychain when it is provably unlocked in
	// a logged-in GUI session, so `security` can never pop an authorization
	// dialog. Any uncertainty (locked keychain, SSH/daemon context, broken
	// probe) skips the CLI quietly instead.
	if !keychainCanBeQueriedSilently() {
		log.Debugf("security: skip %q (keychain not confirmed unlocked in a GUI session)", storage)
		return nil, errors.New("keychain not confirmed unlocked; skipping to stay silent")
	}
...
//  1. GcoredumpRetriever        鈥?CVE-2025-24204 exploit (root only, silent)
//  2. KeychainPasswordRetriever 鈥?direct unlock, needs the login password
//  3. SecurityCmdRetriever      鈥?`security` CLI via the system keychain API
//
// SecurityCmdRetriever is always included: in a logged-in GUI session the login
// keychain is unlocked by default, and browser "Safe Storage" items carry no ACL
// restriction, so `security find-generic-password -wa <label>` returns the secret
// without any dialog. A prompt only occurs when the keychain is locked or the
// process runs outside the user session 鈥?in both cases it fails silently (or is
// denied), which is acceptable for a background run.
