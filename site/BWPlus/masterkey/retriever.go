...
type Hints struct {
	...
	WindowsABEKey  string // Windows ABE browser key (e.g. "chrome"); "" 鈫?ABE not applicable
	LocalStatePath string // path to (temp-copied) Local State JSON; only used on Windows
}

// Retriever obtains a Chromium master key from one platform source (DPAPI, Keychain, D-Bus, 鈥?.
type Retriever interface {
	RetrieveKey(hints Hints) ([]byte, error)
}

// ChainRetriever tries retrievers in order, first success wins (macOS V10: gcoredump鈫抪assword鈫抯ecurity).
